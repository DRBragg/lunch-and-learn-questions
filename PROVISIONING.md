# How to provision a new environment for questions
...
* Enter webroot path: `/home/deployer/apps/questions/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/questions.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/questions.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/questions/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
43 7,19 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/questions/shared/public/
```
